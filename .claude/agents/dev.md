...
- **Resolved commands** (rendered per-repo from the distribution registry):
  - Test/red-green loop command: `./gradlew test`
  - Validate/verify/build command: `./gradlew assembleDebug`
...
- Run with `./gradlew test` (or the repo's resolved test command).
...
### 5. Validate
- Run the repo's validator/build: `./gradlew assembleDebug` (or the
...
- `./gradlew assembleDebug` (or resolved validator) and the full suite pass ✓
